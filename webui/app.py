...
AVAILABLE_MODELS = {
    'kronos-epoch3': {
            'name': 'Kronos-Epoch3 (Best, Local)',
            'model_id': '../epoch_3(best model)',
            'tokenizer_id': 'NeoQuasar/Kronos-Tokenizer-base', # Tokenizer 通常不需要动
            'context_length': 512,
            'params': '102.3M',
            'description': '专利数据微调模型'
        },
    'kronos-mini': {
        'name': 'Kronos-mini',
...
def create_prediction_chart(df, pred_df, lookback, pred_len, actual_df=None, historical_start_idx=0):
    """创建预测图表"""
    # 使用指定的历史数据起始位置，而不是总是从df开头开始
    if historical_start_idx + lookback + pred_len <= len(df):
        # 显示从指定位置开始的lookback个历史点 + pred_len个预测点
    ...
    if pred_df is not None and len(pred_df) > 0:
        ...
        if 'timestamps' in df.columns and len(historical_df) > 0:
            # 从历史数据的最后一个时间戳开始，创建与历史数据相同时间间隔的预测时间戳
    ...
    # 更新布局 - 使用动态参数生成标题
    fig.update_layout(
        title=f'Kronos金融预测结果 - {lookback}个历史点 + {pred_len}个预测点 vs {pred_len}个实际点',
        xaxis_title='时间',
        yaxis_title='价格',
...
def predict():
    ...
    try:
        ...
        if MODEL_AVAILABLE and predictor is not None:
            try:
                ...
                if start_date:
                    ...
                    # 确保有足够数据：lookback + pred_len
                    if len(time_range_df) < lookback + pred_len:
                        return jsonify({'error': f'从开始时间{start_dt.strftime("%Y-%m-%d %H:%M")}起数据不足，至少需要{lookback + pred_len}个数据点，当前仅有{len(time_range_df)}个'}), 400
                    
                    # 使用选定窗口内前lookback个数据点进行预测
                    x_df = time_range_df.iloc[:lookback][required_cols]
                    x_timestamp = time_range_df.iloc[:lookback]['timestamps']
                    
                    # 使用选定窗口内最后pred_len个数据点作为实际值
                    ...
                    prediction_type = f"Kronos模型预测（在选定窗口内：前{lookback}个数据点用于预测，后{pred_len}个数据点用于对比，时间跨度：{time_span}）"
                else:
                    # 使用最新数据
        ...
        if 'timestamps' in df.columns:
            if start_date:
                # 自定义时间段：使用选定窗口数据计算时间戳
                start_dt = pd.to_datetime(start_date)
                mask = df['timestamps'] >= start_dt
                time_range_df = df[mask]
                
                if len(time_range_df) >= lookback:
                    # 从选定窗口的最后一个时间点开始计算预测时间戳
...
def load_model():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': f'模型加载成功: {model_config["name"]} ({model_config["params"]}) 在 {device} 上',
            'model_info': {
...
def get_available_models():
    """获取可用模型列表"""
    # 即使模型实际上不可用，也返回模型列表用于界面显示
    # 这样用户可以看到模型选项并尝试加载
    return jsonify({
        'models': AVAILABLE_MODELS,
        'model_available': True  # 强制返回True以显示模型选项
    })
...
if __name__ == '__main__':
    print("正在启动Kronos Web UI...")
    print(f"模型可用性: {MODEL_AVAILABLE}")
    if MODEL_AVAILABLE:
        print("提示：您可以通过/api/load-model端点加载Kronos模型")
    else:
